...
<div class="content">
  ...
  <ul  class="listhead list_color" v-for="(item,index) in data3">
    ...
    <li>{{item.success_time}}</li>
    <li>{{item.orderName}}</li>
    <li>{{item.phone}}</li>
    <li v-show="item.is_express==1?true:false">是</li>
    <li v-show="item.is_express==1?false:true">否</li>
  </ul>
</div> 
<!-- 分页 -->
<div class="all">
...
<script>
  ...
  methods: {
      ...
      if(_this.value3 != "" && _this.value4 != ""){
        _this.fullscreenLoading = true;
           setTimeout(() => {
            ...
            axios.get(url + "?beginDate="+begindate+"&endDate="+enddate+"&token="+_this.token+"&loginId="+_this.loginId+"&orderStatus="+1)
    ...
    request:function(){
      ...
      checkStatus(_this)
      axios.post(url+"/wcfy/rytj/findInfoByOrderStatus?loginId="+_this.loginId+"&token="+_this.token,
      {
        orderStatus:1,
        pageSize:5,
        pageNum:1
