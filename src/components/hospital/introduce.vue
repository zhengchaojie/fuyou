                ...
                >
                    ...
                    <div>
                        ...
                        <el-select v-model="valueStatus" placeholder="查看体检报告名单">
                            <el-option v-for="v in options1" :key="v.value" :label="v.label"
 ...
 <el-button @click="dialogVisible = false">取 消</el-button>
        ...
        <div class="content">
            ...
            <ul class="listhead font_color" v-for="(item,index) in data3">
                ...
                <li>{{item.billNo}}</li>
                <li v-show='item.sendStatus==1?true:false'>未寄送</li>
                <li v-show="item.sendStatus==2?true:false">寄送中</li>
                <li v-show="item.sendStatus==3?true:false">已签收</li>
                <li  v-show="item.sendStatus==3?true:false">已打印</li>
                <li  v-show="item.sendStatus==1?true:false" @click="returnData(item,index,item.expressId)">
                    打印
                </li>
                <li  v-show="item.sendStatus==2?true:false" @click="returnData(item,index,item.expressId)">
                    再次打印
                </li>
            </ul>
        </div>
...
<script>
    ...
    import {familyDoctor,fyurl, getLastDate, checkStatus} from "../../common/util.js"
    ...
    export default {
        ...
        methods: {
            ...
            requestData: function () {
                ...
                let _this = this
                checkStatus(_this)
                axios.post(url + "/wcfy/sys/express/expressList", {
                    pageSize: 5,
                    pageNum: 1
                }).then(function (response) {
            ...
            termSearch: function () {
                ...
                axios.post(url + "/wcfy/sys/express/expressList?loginId=" + _this.loginId + "&token=" + _this.token,
                    {
            ...
            //下单
            returnData: function (item, index,id) {
                let _this = this;
                checkStatus(_this)
                let url = fyurl();
                this.$confirm("确定寄送快递, 是否继续?", "提示", {
                    ...
                    _this.fullscreenLoading = true;
                    setTimeout(() => {
                        axios.post(url + "/wcfy/sys/express/Order_send", {
                            id:id
                        }).then(response => {
                            if(response.data.code==0){
                                _this.printimg=response.data.img
                                _this.fullscreenLoading = false;
                                $(".header").hide()
                                ...
                                setTimeout(() => {
                                    _this.$confirm('打印机是否出票?', '提示', {
                                        ...
                                        $(".all").show()
                                        $(".footer").show()
                                        $('.main').css("margin-left",'8%')
                                        $('.main').css("padding",'10px')
                                        $("#send_order").hide()

                                        if(_this.beginDate!=''&&_this.startTime!=''){
                                            axios.post(url + "/wcfy/sys/express/expressList?loginId=" + _this.loginId + "&token=" + _this.token,
                                                {
                                                    sendStatus: _this.valueStatus,
                                                    pageSize: 5,
...
<style scoped>
    ...
    .listhead li:nth-of-type(7), .listhead li:nth-of-type(8), .listhead li:nth-of-type(9), .listhead li:nth-of-type(10)
    ,.listhead li:nth-of-type(11){
        width: 12.8%;
        cursor: pointer;
    }
